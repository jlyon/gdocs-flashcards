...
angular.module('intranet', [
        ...
        ['$stateProvider', '$urlRouterProvider',
            function ($stateProvider, $urlRouterProvider) {
                ...
                $stateProvider
                    ...
                    .state("site.intranet", {
                        url: '/intranet',
                        templateUrl: 'views/intranet/intranet.html',
                        auth: true,
                        controller: function ($scope, $rootScope, $state, $filter, $timeout, site) {
                            $scope.site = site;

                            // For social wall
                            $rootScope.socialApi = 'https://feeds.proudcity.com:8443/api/';
                            $rootScope.socialUser = (site.city + ', ' + site.state).replace(/ /g, '_');
                            $rootScope.socialUser = 'San_Rafael,_California'; // @todo: need to save longstate to site
                        }
                    })

                    .state("site.intranet.answers", {
                        url: '/answers',
                        template: '[Embed of Actions app from Intranet]',
                        auth: true,
                        controller: 'SocialController'
                    })

                    .state("site.intranet.social", {
                        url: '/social',
                        template: '<div social-timeline></div>',
                        auth: true,
                        controller: 'SocialController'
                    })

                    .state("site.intranet.directory", {
                        url: '/intranet.directory',
                        templateUrl: 'views/intranet/directory.html',
                        auth: true,
                        controller: function ($scope, $rootScope, $state, $filter, $timeout, site) {
        ...
        function ($scope, SocialFeed, $filter, $sce, $rootScope, $timeout) {
            ...
            self.calculateAspectRatios = function (social, callback) {
                ...
                _.map(social, function (item, key) {
                    // Add template file
                    social[key]['template'] = 'views/intranet/social/default-card-style.html';
                    // Adding to queue
                    imageCount[key] = true;
                    if (item.image) {
    ...
    .directive('socialWall', function factory($window, $rootScope, $browser, $http, $timeout) {
        return {
            restrict: 'A',
            controller: "SocialController",
            templateUrl: 'views/intranet/social/social.html',
    ...
    .directive('socialStatic', function factory($window, $browser, $http, $timeout) {
        return {
            restrict: 'A',
            controller: "SocialController",
            templateUrl: 'views/intranet/social/social-static.html',
            link: function ($scope, $element, $attributes) {
                // Init vars
                $scope.initVars($attributes);
    ...
    .directive('socialTimeline', function factory($window, $browser, $http, $timeout) {
        return {
            restrict: 'A',
            controller: "SocialController",
            templateUrl: 'views/intranet/social/social-timeline.html',
            link: function ($scope, $element, $attributes) {
                // Init vars
                $scope.initVars($attributes);
